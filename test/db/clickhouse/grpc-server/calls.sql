...
CREATE TABLE IF NOT EXISTS test_grpc_server.calls

CREATE TABLE IF NOT EXISTS test_grpc_server.call_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'testing.grpc_server.calls',
    kafka_group_name = 'test_grpc_server_calls_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/grpc/server/call_info:CallInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS test_grpc_server.call_consumer TO test_grpc_server.calls AS
SELECT
    ...
FROM test_grpc_server.call_queue;
