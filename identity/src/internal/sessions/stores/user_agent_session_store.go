...
// If onlyExisting is true, then it returns only existing sessions of user agents.
func (s *UserAgentSessionStore) GetAllByClientId(ctx *actions.OperationContext, clientId uint64, onlyExisting bool) ([]*dbmodels.UserAgentSessionInfo, error) {
	var uass []*dbmodels.UserAgentSessionInfo
	err := s.opExecutor.Exec(ctx, s.opTypes[opTypeUserAgentSessionStore_GetAllByClientId],
		...
		func(opCtx *actions.OperationContext) error {
			...
			if onlyExisting {
				...
				args = []any{clientId, models.UserAgentSessionStatusDeleted}
			} else {
			...
			var err error
			if uass, err = s.store.FindAll(opCtx.Ctx, query, args...); err != nil {
				return fmt.Errorf("[stores.UserAgentSessionStore.GetAllByClientId] find all user agent sessions by client id: %w", err)
			}
			return nil
		},
	)
	if err != nil {
		return nil, fmt.Errorf("[stores.UserAgentSessionStore.GetAllByClientId] execute an operation: %w", err)
	}
	return uass, nil
}

// Exists returns true if the user agent session exists.
func (s *UserAgentSessionStore) Exists(ctx *actions.OperationContext, userId, clientId, userAgentId uint64) (bool, error) {
	var exists bool
	err := s.opExecutor.Exec(ctx, s.opTypes[opTypeUserAgentSessionStore_Exists],
		...
		func(opCtx *actions.OperationContext) error {
			...
			if err != nil {
				return fmt.Errorf("[stores.UserAgentSessionStore.Exists] acquire a connection: %w", err)
			}
			defer conn.Release()

			// FUNCTION: public.user_agent_session_exists(_user_id, _client_id, _user_agent_id) RETURNS boolean
			const query = "SELECT public.user_agent_session_exists($1, $2, $3)"

			if err = conn.QueryRow(opCtx.Ctx, query, userId, clientId, userAgentId).Scan(&exists); err != nil {
				return fmt.Errorf("[stores.UserAgentSessionStore.Exists] execute a query (user_agent_session_exists): %w", err)
			}
			return nil
		},
	)
	if err != nil {
		return false, fmt.Errorf("[stores.UserAgentSessionStore.Exists] execute an operation: %w", err)
	}
	return exists, nil
}
