...
func (m *PermissionManager) GetAllByIds(ctx *actions.OperationContext, ids []uint64) ([]*dbmodels.Permission, error) {
	var ps []*dbmodels.Permission
	err := m.opExecutor.Exec(ctx, iactions.OperationTypePermissionManager_GetAllByIds, []*actions.OperationParam{actions.NewOperationParam("ids", ids)},
		func(opCtx *actions.OperationContext) error {
			if len(ids) == 0 {
			...
			var err error
			if ps, err = m.permissionStore.GetAllByIds(opCtx, ids); err != nil {
				return fmt.Errorf("[manager.PermissionManager.GetAllByIds] get all permissions by ids: %w", err)
			}
			return nil
		},
	)
	if err != nil {
		return nil, fmt.Errorf("[manager.PermissionManager.GetAllByIds] execute an operation: %w", err)
	}
	return ps, nil
}
