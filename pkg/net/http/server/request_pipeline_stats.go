...
type RequestPipelineStats struct {
	...
	countOfResponsesWithErr *uint64 // number of responses with an error that occurred while logging
...
func NewRequestPipelineStats() *RequestPipelineStats {
	return &RequestPipelineStats{
		...
		countOfResponsesWithErr: new(uint64),
...
func (s *RequestPipelineStats) ResponseCount() uint64 {
	return atomic.LoadUint64(s.resCount)
}
...
// CountOfResponsesWithErr returns the number of responses with an error that occurred while logging.
func (s *RequestPipelineStats) CountOfResponsesWithErr() uint64 {
	return atomic.LoadUint64(s.countOfResponsesWithErr)
}
...
func (s *RequestPipelineStats) addResponse() {
	atomic.AddUint64(s.resCount, 1)
}
...
func (s *RequestPipelineStats) addResponseWithError() {
	atomic.AddUint64(s.countOfResponsesWithErr, 1)
}
