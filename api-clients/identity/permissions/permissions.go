...
func (s *PermissionsService) GetAllByNames(ctx *actions.OperationContext, names []string) ([]*permissionspb.Permission, error) {
	ctx2, err := apigrpc.CreateOutgoingContextWithOperationContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("[identity.permissions.PermissionsService.GetAllByNames] create an outgoing context with OperationContext: %w", err)
	}
