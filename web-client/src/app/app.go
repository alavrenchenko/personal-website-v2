...
import (
	...
	wcappconfig "personal-website-v2/web-client/src/app/config"
	clientcontrollers "personal-website-v2/web-client/src/httpcontrollers/clients"
	clientmanager "personal-website-v2/web-client/src/internal/clients/manager"
...
type Application struct {
	...
	configPath        string
	config            *config.AppConfig[*wcappconfig.Apis, struct{}]
	...
	cookieAuthnManager *cookies.CookieAuthnManager
...
func (a *Application) loadConfig() error {
	...
	config := new(config.AppConfig[*wcappconfig.Apis, struct{}])
...
func (a *Application) configureIdentity() error {
	...
	im, err := identity.NewIdentityManager(a.config.UserId, is, wcidentity.Roles, wcidentity.Permissions, a.loggerFactory)
	if err != nil {
...
func (a *Application) configure() error {
	...
	cookieAuthnManager, err := cookies.NewCookieAuthnManager(a.identityManager, ac, a.loggerFactory)
	if err != nil {
		return fmt.Errorf("[app.Application.configure] new cookie authentication manager: %w", err)
	}

	clientManager, err := clientmanager.NewClientManager(a.identityService.Clients, a.identityService.Authentication, a.loggerFactory)
	if err != nil {
		return fmt.Errorf("[app.Application.configure] new client manager: %w", err)
	}

	a.cookieAuthnManager = cookieAuthnManager
	a.clientManager = clientManager
	return nil
}

func (a *Application) configureHttpServer() error {
	rpl, err := apphttpserver.NewRequestPipelineLifetime(a.appSessionId.Value, a.tranManager, a.actionManager, a.identityManager, a.cookieAuthnManager, a.loggerFactory)
	if err != nil {
...
func (a *Application) configureHttpRouting(router *httpserverrouting.Router) error {
	ic := &appcontrollers.ApplicationControllerIdentityConfig{StopPermission: wcidentity.PermissionApp_Stop}
	...
	clientController, err := clientcontrollers.NewClientController(
		a.config.UserId, a.appSessionId.Value, a.actionManager, a.identityManager, *a.cookieAuthnManager, a.clientManager, a.loggerFactory,
	)
	if err != nil {
		return fmt.Errorf("[app.Application.configureHttpRouting] new client controller: %w", err)
	}
	...
	// public
	// api
	router.AddPost("Clients_Init", "/api/clients/init", clientController.Init)
	return nil
}
