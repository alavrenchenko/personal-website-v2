...
CREATE TABLE IF NOT EXISTS website_http_server.requests

CREATE TABLE IF NOT EXISTS website_http_server.request_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'website.http_server.requests',
    kafka_group_name = 'website_http_server_requests_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/request_info:RequestInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS website_http_server.request_consumer TO website_http_server.requests AS
SELECT
    ...
FROM website_http_server.request_queue;
