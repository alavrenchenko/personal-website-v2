...
CREATE TABLE IF NOT EXISTS startup_app_manager_http_server.requests

CREATE TABLE IF NOT EXISTS startup_app_manager_http_server.request_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'startup_app_manager.http_server.requests',
    kafka_group_name = 'startup_app_manager_http_server_requests_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/request_info:RequestInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS startup_app_manager_http_server.request_consumer TO startup_app_manager_http_server.requests AS
SELECT
    ...
FROM startup_app_manager_http_server.request_queue;
