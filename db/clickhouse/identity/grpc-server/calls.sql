...
CREATE TABLE IF NOT EXISTS identity_grpc_server.calls

CREATE TABLE IF NOT EXISTS identity_grpc_server.call_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'identity.grpc_server.calls',
    kafka_group_name = 'identity_grpc_server_calls_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/grpc/server/call_info:CallInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS identity_grpc_server.call_consumer TO identity_grpc_server.calls AS
SELECT
    ...
FROM identity_grpc_server.call_queue;
