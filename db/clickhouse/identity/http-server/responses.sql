...
CREATE TABLE IF NOT EXISTS identity_http_server.responses

CREATE TABLE IF NOT EXISTS identity_http_server.response_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'identity.http_server.responses',
    kafka_group_name = 'identity_http_server_responses_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/response_info:ResponseInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS identity_http_server.response_consumer TO identity_http_server.responses AS
SELECT
    ...
FROM identity_http_server.response_queue;
