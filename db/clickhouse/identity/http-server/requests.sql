...
CREATE TABLE IF NOT EXISTS identity_http_server.requests

CREATE TABLE IF NOT EXISTS identity_http_server.request_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'identity.http_server.requests',
    kafka_group_name = 'identity_http_server_requests_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/request_info:RequestInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS identity_http_server.request_consumer TO identity_http_server.requests AS
SELECT
    ...
