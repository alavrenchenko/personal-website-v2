...
CREATE TABLE IF NOT EXISTS email_notifier_logdb.log

CREATE TABLE IF NOT EXISTS email_notifier_logdb.log_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'email_notifier.log',
    kafka_group_name = 'email_notifier_log_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/logging/log_entry:LogEntry';

CREATE MATERIALIZED VIEW IF NOT EXISTS email_notifier_logdb.log_consumer TO email_notifier_logdb.log AS
SELECT
    ...
FROM email_notifier_logdb.log_queue;
