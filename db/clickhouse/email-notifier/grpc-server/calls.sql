...
CREATE TABLE IF NOT EXISTS email_notifier_grpc_server.calls

CREATE TABLE IF NOT EXISTS email_notifier_grpc_server.call_queue
SETTINGS
    kafka_broker_list = 'localhost:9092',
    kafka_topic_list = 'email_notifier.grpc_server.calls',
    kafka_group_name = 'email_notifier_grpc_server_calls_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/grpc/server/call_info:CallInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS email_notifier_grpc_server.call_consumer TO email_notifier_grpc_server.calls AS
SELECT
    ...
FROM email_notifier_grpc_server.call_queue;
