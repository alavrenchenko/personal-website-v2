...
CREATE TABLE IF NOT EXISTS {name}_grpc_server.calls
(
    id UUID,
...
CREATE TABLE IF NOT EXISTS {name}_grpc_server.call_queue
(
    id String,
...
SETTINGS
    ...
    kafka_topic_list = '{name}.grpc_server.calls',
    kafka_group_name = '{name}_grpc_server_calls_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/grpc/server/call_info:CallInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS {name}_grpc_server.call_consumer TO {name}_grpc_server.calls AS
SELECT
    toUUID(id) AS id,
    app,
    app_session_id,
...
FROM {name}_grpc_server.call_queue;
