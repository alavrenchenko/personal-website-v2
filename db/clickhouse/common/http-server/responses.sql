...
CREATE TABLE IF NOT EXISTS {name}_http_server.responses
(
    id UUID,
...
CREATE TABLE IF NOT EXISTS {name}_http_server.response_queue
(
    id String,
...
SETTINGS
    ...
    kafka_topic_list = '{name}.http_server.responses',
    kafka_group_name = '{name}_http_server_responses_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/response_info:ResponseInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS {name}_http_server.response_consumer TO {name}_http_server.responses AS
SELECT
    toUUID(id) AS id,
    app,
    app_session_id,
    ...
    body_size,
    content_type
FROM {name}_http_server.response_queue;
