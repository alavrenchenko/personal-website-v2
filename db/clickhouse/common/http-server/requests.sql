...
CREATE TABLE IF NOT EXISTS {name}_http_server.requests
(
    id UUID,
...
CREATE TABLE IF NOT EXISTS {name}_http_server.request_queue
(
    id String,
...
SETTINGS
    ...
    kafka_topic_list = '{name}.http_server.requests',
    kafka_group_name = '{name}_http_server_requests_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/net/http/server/request_info:RequestInfo';

CREATE MATERIALIZED VIEW IF NOT EXISTS {name}_http_server.request_consumer TO {name}_http_server.requests AS
SELECT
    toUUID(id) AS id,
    app,
    app_session_id,
...
FROM {name}_http_server.request_queue;
