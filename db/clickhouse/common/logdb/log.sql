...
CREATE TABLE IF NOT EXISTS {name}_logdb.log
(
    id UUID,
...
CREATE TABLE IF NOT EXISTS {name}_logdb.log_queue
(
    id String,
...
SETTINGS
    ...
    kafka_topic_list = '{name}.log',
    kafka_group_name = '{name}_log_clickhouse',
    kafka_format = 'ProtobufSingle',
    kafka_schema = 'personalwebsite/logging/log_entry:LogEntry';

CREATE MATERIALIZED VIEW IF NOT EXISTS {name}_logdb.log_consumer TO {name}_logdb.log AS
SELECT
    toUUID(id) AS id,
